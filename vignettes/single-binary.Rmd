...
cat(sprintf("P(theta > TV  | data) = %.4f  -> Go  criterion (>= 0.80): %s\n",
            p_tv,  ifelse(p_tv  >= 0.80, "YES", "NO")))
cat(sprintf("P(theta > MAV | data) = %.4f  -> NoGo criterion (<= 0.20): %s\n",
            p_mav, ifelse(p_mav <= 0.20, "YES", "NO")))
...
$$\Pr(\mathrm{Go}) =
  ...
  \mathbf{1}\!\left[g_{\mathrm{Go}}(y_1, y_2) \ge \gamma_1\right]\,
  \binom{n_1}{y_1} \pi_1^{y_1}(1-\pi_1)^{n_1-y_1}\,
  \binom{n_2}{y_2} \pi_2^{y_2}(1-\pi_2)^{n_2-y_2},$$

where

$$g_{\mathrm{Go}}(y_1, y_2) = P(\pi_1 - \pi_2 > \mathrm{TV} \mid y_1, y_2),$$
$$g_{\mathrm{NoGo}}(y_1, y_2) = P(\pi_1 - \pi_2 > \mathrm{MAV} \mid y_1, y_2),$$

and the decision outcomes are classified as:

- **Go**: $g_{\mathrm{Go}} \ge \gamma_1$ and $g_{\mathrm{NoGo}} > \gamma_2$
- **NoGo**: $g_{\mathrm{Go}} < \gamma_1$ and $g_{\mathrm{NoGo}} \le \gamma_2$
- **Gray**: all remaining outcomes
...
$$\Pr(\mathrm{NoGo} \mid \gamma) = \sum_{y_1, y_2}
  \mathbf{1}\!\left[g_{\mathrm{NoGo}}(y_1, y_2) \le \gamma\right]\,
  w(y_1, y_2),$$
