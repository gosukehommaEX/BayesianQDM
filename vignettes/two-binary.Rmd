...
The trial enrolls $n_t = n_c = 10$ patients per group in a 1:1 randomised
controlled design.
...
Marginal responder counts: treatment $y_{t,1} = 7$, $y_{t,2} = 5$;
control $y_{c,1} = 4$, $y_{c,2} = 4$.
...
p_post_ctrl <- pbayespostpred2bin(
  ...
  x_t_00 = 2L, x_t_01 = 1L, x_t_10 = 3L, x_t_11 = 4L,
  x_c_00 = 4L, x_c_01 = 2L, x_c_10 = 2L, x_c_11 = 2L,
...
```{r oc-plot, fig.cap = "Operating characteristics (Go/NoGo/Gray) as a function of the true treatment effect on Endpoint 1 (pi_t1 - pi_c1) with equal effects on both endpoints and independence (rho = 0). Controlled design, Jeffreys prior, Exact method. n_t = n_c = 10, gamma_go = gamma_nogo = 0.80."}
...
### 4.4 Effect of Within-Group Correlation

The within-group correlation $\rho_t$ affects the operating characteristics
...
cat(sprintf("Optimal gamma_go*   = %.2f  (Pr(Go) = %.4f)\n",
            res_gamma$gamma_go, res_gamma$PrGo_at_gamma_go))
cat(sprintf("Optimal gamma_nogo* = %.2f  (Pr(NoGo) = %.4f)\n",
            res_gamma$gamma_nogo, res_gamma$PrNoGo_at_gamma_nogo))
```

### 5.3 Figure: Contour Plot of Pr(Go) over $(\gamma_{\mathrm{go}}, \gamma_{\mathrm{nogo}})$ Grid

```{r gamma-contour-go, fig.cap = "Contour plot of Pr(Go) as a function of the candidate threshold pair (gamma_go, gamma_nogo) under the null scenario (pi_t = pi_c = (0.15, 0.15), rho = 0). The 0.05 target contour is highlighted in bold (blue). Dashed lines mark the optimal gamma_go* (blue) and gamma_nogo* (red)."}
g_go   <- res_gamma$gamma_go_grid
g_nogo <- res_gamma$gamma_nogo_grid
Z_go   <- res_gamma$PrGo_grid   # rows = gamma_go, cols = gamma_nogo
...
image(x = g_go, y = g_nogo, z = pmin(Z_go, go_upper),
      breaks = go_breaks, col = go_cols,
      xlab = expression(gamma[go]),
      ylab = expression(gamma[nogo]),
      main = expression("Pr(Go) over (" * gamma[go] * ", " * gamma[nogo] * ") grid"),
      las  = 1,
      useRaster = TRUE)
contour(x = g_go, y = g_nogo, z = Z_go,
...
### 5.4 Figure: Contour Plot of Pr(NoGo) over $(\gamma_{\mathrm{go}}, \gamma_{\mathrm{nogo}})$ Grid

```{r gamma-contour-nogo, fig.cap = "Contour plot of Pr(NoGo) as a function of (gamma_go, gamma_nogo) under the null scenario. The 0.20 target contour is highlighted in bold (red). Dashed lines mark the optimal thresholds."}
...
image(x = g_go, y = g_nogo, z = pmin(Z_nogo, nogo_upper),
      breaks = nogo_breaks, col = nogo_cols,
      xlab = expression(gamma[go]),
      ylab = expression(gamma[nogo]),
      main = expression("Pr(NoGo) over (" * gamma[go] * ", " * gamma[nogo] * ") grid -- Null scenario"),
      las  = 1)
contour(x = g_go, y = g_nogo, z = Z_nogo,
...
cat(sprintf("Optimal gamma_go*   = %.2f  (Pr(Go) = %.4f)\n",
            res_gamma_unctrl$gamma_go, res_gamma_unctrl$PrGo_at_gamma_go))
cat(sprintf("Optimal gamma_nogo* = %.2f  (Pr(NoGo) = %.4f)\n",
            res_gamma_unctrl$gamma_nogo, res_gamma_unctrl$PrNoGo_at_gamma_nogo))
```

### 5.6 Example: Controlled Design, Predictive Probability

> **Note.** When `prob = 'predictive'`, `getgamma2bin()` must evaluate
> `pbayespostpred2bin()` for every outcome combination $(\mathbf{x}_{t,i}, \mathbf{x}_{c,j})$
> using `nMC` Dirichlet draws and $m_j$ future observations each. This
> substantially increases computation time relative to the posterior case.
> The `CalcMethod = 'MC'` option with moderate `nsim` and small `nMC` is
