...
oc_ctrl <- pbayesdecisionprob1cont(
  ...
  kappa0_t = NULL, kappa0_c = NULL, nu0_t = NULL, nu0_c = NULL,
  mu0_t = NULL, mu0_c = NULL, sigma0_t = NULL, sigma0_c = NULL,
  mu_t    = c(1.0, 1.5, 2.0, 2.5, 3.0, 3.5),
  mu_c    = 1.0,
  sigma_t = 2.0,
  sigma_c = 2.0,
  r = NULL,
...
```{r oc-plot, fig.cap = "Operating characteristics (Go/NoGo/Gray) as a function of the true treatment effect (mu_t - mu_c). Controlled design, vague prior, MM method. True mu_c = 1.0, sigma_t = sigma_c = 2.0, n_t = n_c = 15, gamma_go = 0.80, gamma_nogo = 0.20."}
# Compute OC over a finer grid of true treatment means
mu_t_seq <- seq(1.0, 4.5, by = 0.5)
...
delta   <- oc_grid$mu_t - 1.0  # true treatment effect (mu_t - mu_c)
...
plot(delta, oc_grid$Go,
     type = "b", pch = 16, lwd = 2,
     col  = "#2166AC",
     ylim = c(0, 1), xlab = expression(mu[t] - mu[c]),
     ylab = "Probability",
...
- $\gamma_{\mathrm{go}}^*$: selected so that
  ...
  under the null scenario $\mu_t = \mu_c$).
...
res_gamma <- getgamma1cont(
  ...
  theta_TV   = 1.5, theta_MAV = 0.5, theta_NULL = NULL,
  nMC        = NULL,
  mu_t = 1.0, mu_c = 1.0,   # null scenario: no treatment effect
  sigma_t = 2.0, sigma_c = 2.0,
...
```{r gamma-plot, fig.cap = "Pr(Go) and Pr(NoGo) as a function of the candidate threshold gamma. The optimal gamma_go (vertical blue dashed line) is the smallest gamma satisfying Pr(Go) < 0.05 under the null scenario. The optimal gamma_nogo (vertical red dashed line) is the largest gamma satisfying Pr(NoGo) < 0.20 under the null scenario."}
grid_df <- data.frame(
...
if (!is.na(res_gamma$gamma_go)) {
  ...
  text(res_gamma$gamma_go + 0.02, 0.85,
...
if (!is.na(res_gamma$gamma_nogo)) {
  ...
  text(res_gamma$gamma_nogo - 0.02, 0.70,
...
res_gamma_pred <- getgamma1cont(
  ...
  crit_go     = '<',  crit_nogo     = '<',
  sel_go      = 'smallest', sel_nogo = 'largest',
  n_t = 15L, n_c = 15L, m_t = 60L, m_c = 60L,
  kappa0_t = NULL, kappa0_c = NULL, nu0_t = NULL, nu0_c = NULL,
  mu0_t = NULL, mu0_c = NULL, sigma0_t = NULL, sigma0_c = NULL,
  r = NULL,
  ne_t = NULL, ne_c = NULL, alpha0e_t = NULL, alpha0e_c = NULL,
