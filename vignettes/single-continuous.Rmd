...
- Target Value (TV) = 1.5 (ambitious target)
- Minimum Acceptable Value (MAV) = 0.5 (lower bar)
...
Under the vague prior this becomes
...
```{r ctrl-post}
# P(mu1 - mu2 > TV | data) and P(mu1 - mu2 > MAV | data)
...
cat(sprintf("P(theta > TV | data, N-Inv-Chisq prior) = %.4f\n", p_tv_inf))
```
...
cat(sprintf("P(theta > TV | data, uncontrolled) = %.4f\n", p_unctrl))
```
...
cat(sprintf("P(theta > TV | data, external control, alpha=0.5) = %.4f\n",
            p_ext))
```
...
under the Go/NoGo/Gray rule with thresholds $(\gamma_1, \gamma_2)$.
...
$$\widehat{\Pr}(\mathrm{Go}) =
  ...
  \mathbf{1}\!\left[g_{\mathrm{Go}}^{(i)} \ge \gamma_1\right],$$

$$\widehat{\Pr}(\mathrm{NoGo}) =
  \frac{1}{n_{\mathrm{sim}}} \sum_{i=1}^{n_{\mathrm{sim}}}
  \mathbf{1}\!\left[g_{\mathrm{NoGo}}^{(i)} \le \gamma_2\right],$$

$$\widehat{\Pr}(\mathrm{Gray}) = 1 -
  \widehat{\Pr}(\mathrm{Go}) - \widehat{\Pr}(\mathrm{NoGo}),$$

where $g_{\mathrm{Go}}^{(i)} = P(\theta > \mathrm{TV} \mid \mathbf{y}^{(i)})$
and $g_{\mathrm{NoGo}}^{(i)} = P(\theta > \mathrm{MAV} \mid \mathbf{y}^{(i)})$
...
text(0.5,  0.05, "MAV", col = "gray40", cex = 0.8)
text(1.5,  0.05, "TV",  col = "gray40", cex = 0.8)
