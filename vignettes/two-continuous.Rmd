...
cat(sprintf(
  ...
  p_post_vague["R1"], ifelse(p_post_vague["R1"] >= 0.80, "YES -> Go", "NO")
))
cat(sprintf(
  "NoGo region (R9): P = %.4f  >= gamma2 (0.80)? %s\n",
  p_post_vague["R9"], ifelse(p_post_vague["R9"] >= 0.80, "YES -> NoGo", "NO")
))
```
...
cat(sprintf(
  "\nGo  region (R1): P = %.4f  >= gamma1 (0.80)? %s\n",
  p_pred["R1"], ifelse(p_pred["R1"] >= 0.80, "YES -> Go", "NO")
))
```
...
effect on Endpoint 1 ($\mu_{1,1} - \mu_{2,1}$), holding the Endpoint 2
effect fixed at 1.0.

```{r oc-plot, fig.cap = "Operating characteristics (Go/NoGo/Gray) as a function of the true treatment effect on Endpoint 1 (mu1_1 - mu2_1), with Endpoint 2 effect fixed at 1.0. Controlled design, vague prior, MC method. n1 = n2 = 20, gamma1 = gamma2 = 0.80."}
delta1_seq <- seq(0.5, 3.5, by = 0.5)
...
oc_grid <- pbayesdecisionprob2cont(
  ...
  theta.NULL1 = NULL, theta.NULL2 = NULL,
  n1 = 20L, n2 = 20L, m1 = NULL, m2 = NULL,
  mu1    = cbind(delta1_seq, rep(1.0, length(delta1_seq))),
  Sigma1 = Sigma,
...
text(0.5 + 0.05, 0.04, "MAV1", col = "gray40", cex = 0.8, adj = 0)
text(1.5 + 0.05, 0.04, "TV1",  col = "gray40", cex = 0.8, adj = 0)
