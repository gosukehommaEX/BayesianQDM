...
For each marginal component $k = 1, 2$, the difference
$D_k = T_{t,k} - T_{c,k}$ is approximated by a univariate
$t_{\nu_k^*}(\mu_k^*, \sigma_k^*)$ using the same fourth-moment
...
The MM method requires $\nu_{nj} - 1 > 4$ for both groups; if this
...
External data for the control group (sample size $n_{e,c}$, sample mean
$\bar{\mathbf{y}}_{e,c}$, sum-of-squares $S_{e,c}$) are incorporated via
a power prior with weight $\alpha_{0e,c} \in (0,1]$
(arguments `ne_c`, `bar_ye_c`, `se_c`, `alpha0e_c`).

Under the NIW conjugate formulation, the augmented prior for the control
group is first constructed from the external data:
...
$$\boldsymbol{\mu}_{0c}^{\mathrm{aug}} =
  \frac{\kappa_{0c}\,\boldsymbol{\mu}_{0c} +
        \alpha_{0e,c}\,n_{e,c}\,\bar{\mathbf{y}}_{e,c}}
       {\kappa_{0c}^{\mathrm{aug}}},$$

$$\Lambda_{0c}^{\mathrm{aug}} = \Lambda_{0c} + \alpha_{0e,c}\,S_{e,c} +
  \frac{\kappa_{0c}\,\alpha_{0e,c}\,n_{e,c}}
       {\kappa_{0c}^{\mathrm{aug}}}
  (\bar{\mathbf{y}}_{e,c} - \boldsymbol{\mu}_{0c})
  (\bar{\mathbf{y}}_{e,c} - \boldsymbol{\mu}_{0c})^\top.$$
...
Treatment effect scenarios are defined by varying $\boldsymbol{\mu}_t$
(`mu_t`) while fixing $\boldsymbol{\mu}_c = (0, 0)^\top$ (`mu_c`):
...
oc_ctrl <- pbayesdecisionprob2cont(
  ...
  mu_t = rbind(c(0.5, 0.3), c(1.5, 1.0), c(2.5, 1.5), c(3.5, 2.0)),
  Sigma_t = Sigma,
  mu_c = rbind(c(0.0, 0.0), c(0.0, 0.0), c(0.0, 0.0), c(0.0, 0.0)),
  Sigma_c = Sigma,
  kappa0_t = NULL, nu0_t = NULL, mu0_t = NULL, Lambda0_t = NULL,
  kappa0_c = NULL, nu0_c = NULL, mu0_c = NULL, Lambda0_c = NULL,
  r = NULL,
...
effect on Endpoint 1 ($\mu_{t,1} - \mu_{c,1}$). Both endpoints increase
together ($\mu_{t,2} = 0.6 \times \mu_{t,1}$) to show the transition
from NoGo through Gray to Go clearly.

```{r oc-plot, fig.cap = "Operating characteristics (Go/NoGo/Gray) as a function of the true treatment effect on Endpoint 1. Both endpoints increase proportionally (mu_t2 = 0.6 * mu_t1). Controlled design, vague prior, MC method. n_t = n_c = 20, gamma_go = 0.80, gamma_nogo = 0.20."}
...
oc_grid <- pbayesdecisionprob2cont(
  ...
  theta_NULL1 = NULL, theta_NULL2 = NULL,
  n_t = 20L, n_c = 20L, m_t = NULL, m_c = NULL,
  mu_t    = cbind(delta1_seq, 0.6 * delta1_seq),
  Sigma_t = Sigma,
  mu_c    = matrix(0, nrow = length(delta1_seq), ncol = 2),
  Sigma_c = Sigma,
  kappa0_t = NULL, nu0_t = NULL, mu0_t = NULL, Lambda0_t = NULL,
  kappa0_c = NULL, nu0_c = NULL, mu0_c = NULL, Lambda0_c = NULL,
  r = NULL,
...
plot(delta1_seq, oc_grid$Go,
     ...
     ylim = c(0, 1),
     xlab = expression(mu[paste("t,1")] - mu[paste("c,1")]),
     ylab = "Probability",
...
oc_unctrl <- pbayesdecisionprob2cont(
  ...
  mu_c    = rbind(c(0.0, 0.0), c(0.0, 0.0), c(0.0, 0.0), c(0.0, 0.0)),
  Sigma_c = Sigma,
  kappa0_t = 2.0, nu0_t = 5.0, mu0_t = c(2.0, 1.0), Lambda0_t = L0oc,
  kappa0_c = NULL, nu0_c = NULL, mu0_c = c(0.0, 0.0), Lambda0_c = NULL,
  r = 1.0,
  ne_t = NULL, ne_c = NULL, alpha0e_t = NULL, alpha0e_c = NULL,
  bar_ye_t = NULL, bar_ye_c = NULL, se_t = NULL, se_c = NULL,
...
```{r oc-external}
Se_c_oc <- matrix(c(7.0, 1.2, 1.2, 1.8), 2, 2)
...
oc_ext <- pbayesdecisionprob2cont(
  ...
  Sigma_c = Sigma,
  kappa0_t = 2.0, nu0_t = 5.0, mu0_t = c(2.0, 1.0), Lambda0_t = L0oc,
  kappa0_c = 2.0, nu0_c = 5.0, mu0_c = c(0.0, 0.0), Lambda0_c = L0oc,
  r = NULL,
  ne_t = NULL, ne_c = 15L, alpha0e_t = NULL, alpha0e_c = 0.5,
  bar_ye_t = NULL, bar_ye_c = c(0.2, 0.1), se_t = NULL, se_c = Se_c_oc,
  nMC = 500L, CalcMethod = 'MC',
...
are generated from the scenario $(\boldsymbol{\mu}_t, \boldsymbol{\mu}_c,
\Sigma_t, \Sigma_c)$.  For each replicate $i$,
...
res_gamma <- getgamma2cont(
  ...
  prior = 'vague',
  GoRegions = 1L, NoGoRegions = 9L,
  mu_t = c(0.5, 0.3), Sigma_t = Sigma,   # null: effect at MAV boundary
  mu_c = c(0.0, 0.0), Sigma_c = Sigma,
...
cat(sprintf("Optimal gamma1* = %.2f  (Pr(Go) = %.4f)\n",
            res_gamma$gamma_go, res_gamma$PrGo_at_gamma_go))
cat(sprintf("Optimal gamma2* = %.2f  (Pr(NoGo) = %.4f)\n",
            res_gamma$gamma_nogo, res_gamma$PrNoGo_at_gamma_nogo))
```

### 6.3 Figure: Contour Plot of Pr(Go) over $(\gamma_1, \gamma_2)$ Grid

```{r gamma-contour-go, fig.cap = "Contour plot of Pr(Go) as a function of the candidate threshold pair (gamma1, gamma2) under the null scenario (mu_t = (0.5, 0.3), mu_c = (0, 0)). The dashed lines mark the optimal gamma1* (blue) and gamma2* (red). The 0.05 contour is highlighted in bold."}
g1   <- res_gamma$gamma_go_grid
g2   <- res_gamma$gamma_nogo_grid
...
if (!is.na(res_gamma$gamma_go))
  abline(v = res_gamma$gamma_go, lty = 2, lwd = 2, col = "#2166AC")
if (!is.na(res_gamma$gamma_nogo))
  abline(h = res_gamma$gamma_nogo, lty = 2, lwd = 2, col = "#D6604D")
legend("topright",
       legend = c(
         paste0("gamma1* = ", res_gamma$gamma_go),
         paste0("gamma2* = ", res_gamma$gamma_nogo),
         "Pr(Go) = 0.05 contour"),
...
cat(sprintf("Optimal gamma1* = %.2f  (Pr(Go) = %.4f)\n",
            res_gamma_pred$gamma_go, res_gamma_pred$PrGo_at_gamma_go))
cat(sprintf("Optimal gamma2* = %.2f  (Pr(NoGo) = %.4f)\n",
            res_gamma_pred$gamma_nogo, res_gamma_pred$PrNoGo_at_gamma_nogo))
```
...
4. Three study designs: controlled, uncontrolled ($\boldsymbol{\mu}_{0c}$
   and $r$ fixed), external control (NIW power prior with explicit augmentation
