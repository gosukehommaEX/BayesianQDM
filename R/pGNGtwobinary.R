...
#'        used inside \code{\link{pPPtwobinary}} for each count combination.
...
#' }
#' The returned object has S3 class \code{pGNGtwobinary} with an associated
#' \code{print} method.
#'
#' @details
...
#' \eqn{(x_t, x_c)} are enumerated using \code{\link{allCountsBinary}}.
#' For each combination, \code{\link{pPPtwobinary}} computes the Go and NoGo
...
#' \code{\link{getJointBinary}} converts the marginal rates and correlation
...
#' @export
pGNGtwobinary <- function(prob        = 'posterior',
                          design      = 'controlled',
                          GoRegions,
                          NoGoRegions,
                          gamma1,
  ...
  if (prob == 'posterior') {
    ...
    # For pPPtwobinary, predictive uses TV = MAV = NULL threshold
  ...
  counts_t <- allCountsBinary(n1)   # (n_t x 4) integer matrix
  counts_c <- allCountsBinary(n2)   # (n_c x 4) integer matrix
  n_t      <- nrow(counts_t)
  n_c      <- nrow(counts_c)
  ...
  for (s in seq_len(n_scen)) {
    ...
    p_t <- getJointBinary(pi1 = pi_t1[s], pi2 = pi_t2[s], rho = rho_t[s])
    ...
    if (design == 'uncontrolled') {
      ...
      # conceptually -- but pPPtwobinary draws from Dir(a2 + z) regardless
      ...
      p_c <- getJointBinary(pi1 = pi_c1[s], pi2 = pi_c2[s], rho = rho_c[s])
  ...
  class(results) <- c('pGNGtwobinary', 'data.frame')
...
#' Print Method for pGNGtwobinary Objects
#'
#' Displays a formatted summary of Go/NoGo/Gray decision probabilities for
#' two-binary-endpoint results returned by \code{\link{pGNGtwobinary}}.
#'
#' @param x An object of class \code{pGNGtwobinary}.
...
#'
#' @export
print.pGNGtwobinary <- function(x, digits = 4, ...) {
