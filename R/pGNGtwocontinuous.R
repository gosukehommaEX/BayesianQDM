...
#' \code{\link{pPPtwocontinuous}}, which returns the nine (posterior) or four
...
#'        passed to \code{\link{pPPtwocontinuous}} for each simulated dataset.
...
#' }
#' The object has class \code{c('pGNGtwocontinuous', 'data.frame')} and
...
#'   \item \code{\link{pPPtwocontinuous}} is called to obtain region
...
#' @export
pGNGtwocontinuous <- function(nsim,
                              prob, design, prior,
  ...
  # Threshold validation (delegated to pPPtwocontinuous; check basics here)
  if (prob == 'posterior') {
  ...
  for (s in seq_len(n_scen)) {
    ...
    for (i in seq_len(nsim)) {
      ...
      # Call pPPtwocontinuous to get region probability vector
      Pr_R <- pPPtwocontinuous(
  ...
  class(results) <- c('pGNGtwocontinuous', 'data.frame')
...
#' Print Method for pGNGtwocontinuous Objects
#'
#' Displays a formatted summary of Go/NoGo/Gray decision probabilities for
#' two-continuous-endpoint results returned by \code{\link{pGNGtwocontinuous}}.
#'
#' @param x An object of class \code{pGNGtwocontinuous}.
...
#'
#' @export
print.pGNGtwocontinuous <- function(x, digits = 4, ...) {
