...
#' @export
pPPtwocontinuous <- function(prob,
                             design,
                             prior,
                             theta.TV1   = NULL, theta.MAV1  = NULL,
                             theta.TV2   = NULL, theta.MAV2  = NULL,
  ...
  if (prob == 'posterior') {
    ...
    mu_t <- rMVT(nMC, df = post1$df, mu = post1$mu_n, V = post1$V)
    mu_c <- rMVT(nMC, df = post2$df, mu = post2$mu_n, V = post2$V)
    ...
    ytilde_t <- rMVT(nMC, df = post1$df, mu = post1$mu_n, V = V1_pred)
    ytilde_c <- rMVT(nMC, df = post2$df, mu = post2$mu_n, V = V2_pred)
